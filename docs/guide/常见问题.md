...
Embykeeper0 需要运行内存为 100-300 MB, 这是因为运行过程中, 部分 Bot 签到需要进行验证码识别. 如果您在运行过程中有内存不足的情况, 请适当调高您的虚拟内存.

如果依然无法使用, 请适当调高虚拟内存, 或更换内存更大的设备.
...
```bash
docker run -v $(pwd)/embykeeper:/app --rm -it --net=host ghcr.io/dotracel/emby-keeper0 -i -CL
```
...
- 自定义启用的站点: [教程](/guide/配置文件#site-子项)
- 自定义 Emby 站点的 User-Agent: [教程](/guide/配置文件#emby-%E5%AD%90%E9%A1%B9)
- 自定义某些站点的自定义项: [教程](/guide/配置文件#自定义签到站点)

如果有其他您需要自定义的项目, 请通过 [GitHub Issues](https://github.com/DotRacel/emby-keeper0/issues) 反馈.
...
若您的疑问没有得到解答, 您可以通过 [GitHub Issues](https://github.com/DotRacel/emby-keeper0/issues) 进行反馈.

如果您发现一个程序上的错误或问题, 您也可以通过 [GitHub Issues](https://github.com/DotRacel/emby-keeper0/issues) 进行反馈.

### 反馈时如何保护隐私?

请不要在截图或反馈信息中包含手机号、Telegram UID、用户名、服务器地址、账号密码等个人隐私或凭据. 如需提供日志, 请先打码后再通过 GitHub Issues 反馈.
...
3. **进一步挖掘**: 我们对我们使用的库 `pyrogram` 进行相关问题分析, 发现了大量反馈, 同时其他库 (`telethon`) 也有类似的反馈.
   ...
   相关问题记录在: [GitHub Issue](https://github.com/DotRacel/emby-keeper0/issues/67#issuecomment-2543344275) 中.
...
- `device`: 设备名, 自 [v2.6.0](https://github.com/DotRacel/emby-keeper0/releases/tag/v2.6.0) 后保持不变, 自 [v7.0.0](https://github.com/DotRacel/emby-keeper0/releases/tag/v7.0.0) 后转为带缓存生成的形式, 同一站点保持不变.
...
你可以查看[源代码](https://github.com/DotRacel/emby-keeper0/blob/main/embykeeper/emby/main.py)以更清晰地了解逻辑.

### 部分需要 Cloudflare (CF) 验证码的站点为什么失败?

本维护版本已经移除在线验证码解析服务. 如果目标站点强制要求 Cloudflare 验证, Embykeeper0 不会再尝试调用远端服务跳过验证, 对应站点可能无法自动完成签到或保活.
